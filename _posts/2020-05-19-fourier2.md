...
Ce procédé permet de lisser les irrégularités de la fonction de base et définit de façon plus "concrète" la notion de fonction. En effet, dans la vraie vie (celle de la physique), lorsqu'on veut mesurer quelque chose on dispose de capteurs qui ne sont pas parfaits. 
...
$\mathbb{S}(\mathbb{R})$ étant un espace de fonction on peut définir une notion de continuité propres aux fonctions de l'espace de Schwartz.
...
On peut voir n'importe quelle fonction un peu régulière comme une distribution. Dans l'exemple de la température la distribution dépendait d'une fonction $f$ (notre capteur). On parle ici de *distribution tempérée associée à f*:
...
**Remarque 3.** Une distribution tempérée définit bien une distribution. Elle est clairement linéaire en $\phi$ et elle est bien continue en $\phi$. En effet si $\phi_m \rightarrow \phi$ alors en notant $I_m=\lvert T_f(\phi_m)-T_f(\phi) \rvert$:
\begin{equation}
I_m\leq \int_{\mathbb{R}} \lvert f(x) \rvert \lvert \phi_m(x)-\phi(x) \rvert dx \leq \int_{\mathbb{R}} C(f) (1+x^{2}) \lvert \phi_m(x)-\phi(x) \rvert dx
\end{equation}
Le membre de droite est intégrable car par combinaison linéaire $\phi_m-\phi$ est aussi de Schwartz et on peut faire le même raisonnement que dans la remarque précédente. On peut donc appliquer le théorème de convergence dominé et intervertir limite et intégrale pour conclure que $\lim_{m\rightarrow+\infty} I_m \leq  \int_{\mathbb{R}} \lvert f(x) \rvert \lim_{m\rightarrow+\infty} \lvert \phi_m(x)-\phi(x) \rvert dx=0$.
...
\begin{equation}
\forall \phi \in \mathbb{S}(\mathbb{R}), \text{ } \langle \mathcal{F}(T_f), \phi \rangle= \langle T_f, \mathcal{F}(\phi) \rangle
\end{equation}
