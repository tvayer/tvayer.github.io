...
\begin{equation}
\lim_{T \rightarrow \infty} T c_n(f)=\int_{-\infty}^{+\infty} f(t) e^{-i n \omega t}dt
\end{equation}
...
On appelle ces courbes les représentations fréquentielles de nos signaux ou encore les spectres des signaux. Elles sont une autre manière de représenter nos signaux, et, souvent permettent une meilleur analyse de ces signaux. 

#### La transformée de Fourier inverse <a name="invfour"></a>

Comme pour les séries de Fourier on peut se demander si on peut retrouver la fonction $f$ au travers de sa transformée de Fourier afin d'avoir cette notion de "dualité" entre ces deux objets. Dans les séries de Fourier on avait souvent l'égalité entre la série de Fourier et la fonction. Ici la dualité va exister au travers de la notion de *transformée de Fourier inverse*. A l'aide d'une opération sur la transformée de Fourier on va pouvoir retrouver le signal original.
