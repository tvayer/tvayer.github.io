...
ax.set_title('Monthly avg temperature of {0}'.format(
    country_name), fontsize=fs+3)
...
for i, ratio in enumerate([0.5, 0.8]):
    ...
    X_train, X_test = X[0:train_idx,], X[train_idx:,]
    y_train, y_test = y[0:train_idx], y[train_idx:]

    kernel_ridge = KernelRidge(kernel="poly", degree=2, alpha=1, coef0=1)
#     kernel_ridge = KernelRidge(kernel="rbf", gamma=1e-5, alpha=1e-4)
...
fig.suptitle('Monthly avg temperature of {0}'.format(
    country_name), fontsize=fs+3)
fig.tight_layout()
...
all_gammas = np.logspace(-8, -2, 20)
all_alphas = np.logspace(-6, 3, 20)
...
for dic_param in param_grid:
    ...
    error[(dic_param['gamma'], dic_param['alpha'])
          ] = np.linalg.norm(ypred-y_test)
# %%

zz = np.array(list(error.values())).reshape(
    all_gammas.shape[0], all_alphas.shape[0])
fs = 15
...
print('Lowest error for (gamma, reg) = {0}, value = {1:.3f}'.format(
    best_, error[best_]))
gamma_best, alpha_best = best_[0], best_[1]
kernel_ridge_best = KernelRidge(
    kernel="rbf", gamma=gamma_best, alpha=alpha_best)
...
# %%
all_params = {'w0': np.logspace(-4, 4, 30),
              'K': [3, 5, 10, 20, 50, 100],
...
for dic_param in param_grid:
    ...
    instantiate_modeled.fit(X_train, y_train)
    ypred = instantiate_modeled.predict(X_test)
    error[(dic_param['w0'], dic_param['K'], dic_param['alpha'])
          ] = np.linalg.norm(ypred-y_test)
# %%
best_ = min(error, key=error.get)
print('Lowest error for (w0, K, alpha) = {0}, value = {1:.3f}'.format(
    best_, error[best_]))
w0_best, K_best, alpha_best = best_[0], best_[1], best_[2]
best_seasonality_model = SeasonalityKernel(
    w0=w0_best, K=K_best, alpha=alpha_best)
# %%
fs = 10
fig, ax = plt.subplots(1, 1, figsize=(5, 5))
...
ax.axvspan(temp.date[0], temp.date[train_idx],
           facecolor='blue', label='Training set', alpha=0.1)
ax.axvspan(temp.date.values[-1], date_all[-1],
           facecolor='red', label='Projection', alpha=0.1)
...
all_params = {'w0': np.logspace(-4, 3, 30),
              'K': [3, 5, 10, 20, 50, 100],
              'alpha': np.logspace(-6, 3, 10)}
...
fs = 10
fig, ax = plt.subplots(1, 1, figsize=(8, 5))
ax.axvspan(temp.date[0], temp.date[split_train_test],
           facecolor='blue', label='Train/Valid set', alpha=0.1)
ax.axvspan(temp.date.values[-1], date_all[-1],
           facecolor='red', label='Projection', alpha=0.1)
